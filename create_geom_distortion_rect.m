function [im_out] = create_geom_distortion_rect( ...
    im_size, ...
    Shape_area_part, ...
    relative_w_h, ...
    d_h_max_perc, ...
...
N_max = 5; % максимальное количестко точек
%d_h_max_perc = 0.5; % максимальное отклонение координаты в случайных точках
%d_ij_Nodes_max_perc = 1.5;% максимальное отклонение координат вершин фигуры
...
rect_w = sqrt(S_shape * relative_w_h);
rect_h = rect_w/relative_w_h;
rect_w = round(rect_w);
rect_h = round(rect_h);
if rect_w > m || rect_h > n
    disp('Фигура выходит за границы изображения')
end

%% формирование номинальных координат прямоугольника
i0 = round(n/2);
j0 = round(m/2);

I_arr = zeros(1,4);
J_arr = zeros(1,4);
%обход по часовой стрелке начиная с левой верхней
I_arr(1) = -rect_h/2;
J_arr(1) = -rect_w/2;
...
I_arr = round(I_arr) + i0;
J_arr = round(J_arr) + j0;
...
dI_arr = round(-d_ij_Nodes_max + 2*d_ij_Nodes_max*rand(1,4));
dJ_arr = round(-d_ij_Nodes_max + 2*d_ij_Nodes_max*rand(1,4));

I_dist = I_arr + dI_arr;
J_dist = J_arr + dJ_arr;
%% вычисление углов прямоугольника с погрешностью положения вершин
alpha = zeros(1,4); %углы при одноименных вершинах
vec12 = form_vec(I_dist,J_dist,1,2);%вектор от точки 1 к точке 2.
vec14 = form_vec(I_dist,J_dist,1,4);
...
vec41 = form_vec(I_dist,J_dist,4,1);
vec43 = form_vec(I_dist,J_dist,4,3);
alpha(4) = calc_angle_betw_vecs(vec41,vec43);
...
for k = 1:4 %!!!
    ...
    if k+1<=4 % у последнего ребра вторая точка в общем порядке стоит первой
