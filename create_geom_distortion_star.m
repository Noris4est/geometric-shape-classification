function im_out = create_geom_distortion_star( ...
    im_size, ...
    OuterDiamRelativeImDiag, ...
    Nodes_Number)
%% Априорные данные
N_min = 1; % минимальное количество точек (не включая две крайние по умолчанию 0)
N_max = 4; % максимальное количестко точек
d_h_max_perc = 0.4; % Локальная ошибка контура в %
d_ij_Nodes_max_perc = 2;% Ошибка положения вершин в %
%отклонение может быть в + и -. Задается в % от диагонали кадра.
%% Расчет геометрических характеристик
n = im_size(1);
m = im_size(2);
diag_pix = sqrt(n^2 + m^2);
outerRadius = OuterDiamRelativeImDiag*diag_pix/2; %радиус внешней окружности
if outerRadius > n/2 || outerRadius > m/2
    disp('Фигура выходит за границы изображения')
end

%особенная формула связи внешнего и внутреннего радиусов
s1 = sin(pi/Nodes_Number);
t1 = tan(pi/2 - pi/Nodes_Number);
t2 = tan(2*pi/Nodes_Number);
innerRadius = outerRadius/(s1*(t1+t2)); %радиус внутренней окружности
%% формирование номинальных координат звезды
i0 = round(n/2);
j0 = round(m/2);
d_phi = 2*pi/Nodes_Number;
phi_outer = 0 : d_phi : 2*pi - d_phi; %угловые координаты вершин многоугольника
phi_inner = d_phi/2 : d_phi : 2*pi - d_phi/2;

I_outer_arr = - sin(phi_outer)*outerRadius + i0;
J_outer_arr = cos(phi_outer)*outerRadius + j0;

I_inner_arr = - sin(phi_inner)*innerRadius + i0;
J_inner_arr = cos(phi_inner)*innerRadius + j0;


I_arr = zeros(1,2*Nodes_Number);
J_arr = zeros(1,2*Nodes_Number);


I_arr(1:2:end) = I_outer_arr;
I_arr(2:2:end) = I_inner_arr;

J_arr(1:2:end) = J_outer_arr;
J_arr(2:2:end) = J_inner_arr;
...
dI_arr = round(-d_ij_Nodes_max + 2*d_ij_Nodes_max*rand(1,2*Nodes_Number));
dJ_arr = round(-d_ij_Nodes_max + 2*d_ij_Nodes_max*rand(1,2*Nodes_Number));

I_dist = I_arr + dI_arr;
J_dist = J_arr + dJ_arr;
%% вычисление углов 
alpha = zeros(1,2*Nodes_Number); %углы при одноименных вершинах
vec12 = form_vec(I_dist,J_dist,1,2);%вектор от точки 1 к точке 2.
vec1_last = form_vec(I_dist,J_dist,1,2*Nodes_Number);
alpha(1) = calc_angle_betw_vecs(vec12,vec1_last);
...
vec_N_to_1 = form_vec(I_dist,J_dist,Nodes_Number*2,1);
vec_N_to_prev = form_vec(I_dist,J_dist,Nodes_Number*2,Nodes_Number*2 - 1);
...
for k = 1:2*Nodes_Number %!!!
    p1 = [I_dist(k) J_dist(k)]; %первая точка рассм. ребра
    alpha1 = alpha(k); %первый примыкающий угол
    if k+1<=2*Nodes_Number % у последнего ребра вторая точка в общем порядке стоит первой
